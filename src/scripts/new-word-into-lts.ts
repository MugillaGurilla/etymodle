...
class NewWord {
    ...
    private writeData() : void {
        ...
        fs.readdir("./answer/all/", (err, files) => {
            ...
            let biggest = 1;
            files.map((file) => {
                const current = parseInt(file.split(".")[0]);
                if (current > biggest) {
                    biggest = current;
                }
            });
            fs.writeFile(`./answer/all/${biggest + 1}.json`, JSON.stringify(data, null, 2), (err) => {
                if (err) {
                    console.error("Error writing to file:", err);
                } else {
                    console.log(`Data successfully written to ${biggest + 1}.json`);
                }
            });
        });
